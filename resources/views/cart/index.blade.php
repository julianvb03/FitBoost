@extends('layouts.app')
@section('title', trans('cart/index.title'))

@section('content')
    <div class="space-y-8">
        <div class="flex items-center justify-between">
            <h1 class="text-4xl font-bold text-base-content">{{ trans('cart/index.cart_title') }}</h1>
            @if ($viewData['count'] > 0)
                <span class="badge badge-primary badge-lg text-lg px-4 py-3">{{ trans('cart/index.items_count', ['count' => $viewData['count']]) }}</span>
            @endif
        </div>
        ...
        @if ($viewData['count'] === 0)
            <div class="card bg-base-100 shadow-xl">
                <div class="card-body items-center text-center py-16">
                    ...
                    <h2 class="card-title text-3xl mt-6">{{ trans('cart/index.empty_cart') }}</h2>
                    <p class="text-base-content/70 mb-8 text-lg">{{ trans('cart/index.empty_cart_description') }}</p>
                    <a class="btn btn-primary btn-lg" href="{{ route('supplements.index') }}">
                        {{ trans('cart/index.continue_shopping') }}
                    </a>
                </div>
            </div>
        @else
                            ...
                            <div class="flex flex-col lg:flex-row gap-8 items-center lg:items-start">
                                ...
                                <div class="flex-1 w-full">
                                    ...
                                    <p class="text-base-content/60 text-lg mb-4">
                                        {{ trans('cart/index.stock') }}: <span class="font-semibold">{{ $supplement->getStock() }}</span>
                                    </p>
                                ...
                                <div class="flex flex-col lg:flex-row items-center gap-6 w-full lg:w-auto">
                                    <div class="flex items-center gap-4">
                                        <form method="POST"
                                            ...
                                            <div class="flex items-center gap-3">
                                                <label class="text-lg font-semibold">{{ trans('cart/index.quantity') }}:</label>
                                    ...
                                    <div class="flex flex-col items-center gap-4">
                                        <div class="text-xl font-semibold text-base-content/60">{{ trans('cart/index.subtotal') }}</div>
                                    ...
                                    <div>
                                        <form method="POST"
                                            ...
                                            onsubmit="return confirm('{{ trans('cart/index.confirm_remove') }}');">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class="flex flex-col lg:flex-row gap-6 mt-8">
                <div class="flex-1">
                    <form method="POST" action="{{ route('cart.clear') }}"
                        onsubmit="return confirm('{{ trans('cart/index.confirm_empty') }}');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-outline btn-error btn-lg">
                            ...
                            {{ trans('cart/index.empty_cart_button') }}
                        </button>
                    </form>
                </div>
                        ...
                        <div class="card-actions">
                            ...
                            @else
                                <a href="{{ route('login') }}" class="btn btn-success btn-lg btn-block">
                                    ...
                                    {{ trans('cart/index.login_to_checkout') }}
                                </a>
                            @endauth
                        </div>
                    </div>
