            ...
            <tbody>
                @foreach ($items as $row)
                    ...
                    <tr>
                        ...
                        <td>
                            <form class="d-inline" method="POST" action="{{ route('cart.items.update', ['supplement' => $supplement->getId()]) }}">
                                @csrf
                                @method('PATCH')
                                <div class="input-group">
                                    <input name="quantity" type="number" min="1" max="99" class="form-control" value="{{ $quantity }}">
                        ...
                        <td>
                            <form class="d-inline" method="POST" action="{{ route('cart.items.destroy', ['supplement' => $supplement->getId()]) }}">
                                @csrf
                                @method('DELETE')
